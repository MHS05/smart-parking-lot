...
while True :
    ...
    model = torch.hub.load('C:\AI_Parking_System/yolov5',
                           'custom', 
                           path='C:\AI_Parking_System/yolov5/runs/train/numberplate_results/weights/best.pt', 
    ...
    if dbms.DBOpen("192.168.0.101", 3306, "sps", "root", "ezen") == False :
        print("ERROR")
    sql = f"select * from carinfo where carnum = 'N'" 
    dbms.OpenQuery(sql)
    # 전체 데이터 갯수
    total = dbms.GetTotal()
    print("전체 데이터 갯수 : ", total)
    # cmno의 목록
    cmno = []
    ...
    def getEnterpic(cmno) :
        dbms = db.DBmanager() 
        if dbms.DBOpen("192.168.0.101", 3306, "sps", "root", "ezen") == False :
            print("ERROR")
            return False
        sql = f"select * from carinfo where cmno = {cmno}"
        #print(sql)
        dbms.OpenQuery(sql)
        enterpic = dbms.GetValue(0, "enterpic")
    ...
    for j in cmno :
        #print(j)
        enterpic = getEnterpic(j)
        #enterpic, cmno, carnum = getEnterpic(i)
        print("cmno : ", j)
        print("enterpic : ", enterpic)
    ...
    def easy_ocr (path) :
        ...
        if dbms.DBOpen("192.168.0.101", 3306, "sps", "root", "ezen") == False :
           print("ERROR")
           return False
        sql = "update carinfo set carnum = '" + result + "' where cmno = '" + str(j) + "'"
        print(sql)
        dbms.RunSql( sql )
        dbms.DBClose()
